...
################################################
bpg = BinaryPursuitGame(nx_graph_atk, nx_graph_def,
                            start_atk, start_def, target_values, depth, 'exact_edge') # class closeness not used
clg = LayeredGame(bpg.atk_adj, bpg.def_adj, target_values,
...
for path_min_idx in range(num_paths_minimizer):
    for path_max_idx in range(num_paths_maximizer):
        game_matrix[path_min_idx][path_max_idx] = 0.0 if path_clash(clg,
                paths_maximizer[path_max_idx], paths_minimizer[path_min_idx]) else target_values[paths_maximizer[path_max_idx][-1]]
